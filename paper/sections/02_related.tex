...
Early research into LLM-driven testing set out to see if models could output viable unit tests on the first try, bypassing any post-generation debugging. For example, Guilherme and Vincenzi~\cite{guilherme2023sast} prompt-engineered \texttt{gpt-3.5-turbo} (using a low temperature of 0.2) to write tests for 33 Java programs. Although they saw high averages in code coverage (90.2\%) and mutation scores (70.5\%), only 64.6\% of the tests compiled and ran successfully. This compilation bottleneck became a common theme in subsequent studies. Sch{\"a}fer et al.~\cite{schafer2024testpilot} evaluated \texttt{gpt-3.5-turbo}, \texttt{code-cushman-002}, and StarCoder on JavaScript, TypeScript, and Python functions using their TestPilot tool. While they reached a median statement coverage of 70.2\% and branch coverage of 52.8\%, they reported that syntax errors and unmocked external dependencies frequently broke the generated test suites.
